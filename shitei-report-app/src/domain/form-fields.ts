...
export const CHECK_FIELDS = ['rt_kal', 'rt_phy', 'rt_tel', 'rt_vis', 'rt_opn'] as const;

export type FormFieldId = (typeof FORM_FIELDS)[number];
export type CheckFieldId = (typeof CHECK_FIELDS)[number];

export type CaseData = Record<string, string | boolean>;

export function emptyCaseData(): CaseData {
  const data: CaseData = {};
...
export function getCharCount(data: CaseData): number {
...
export function parseIcdNum(icd: string): number | null {
