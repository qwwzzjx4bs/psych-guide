...
function firstUnfilledAmongFields(doc, fieldIds) {
  for (let n = 1; n <= 5; n++) {
...
function firstCharCountIssue(doc) {
  for (let n = 1; n <= 5; n++) {
    const count = getCharCount(getCase(doc, n));
...
function firstRestrictionDetailNeeded(doc) {
  for (let n = 1; n <= 5; n++) {
    const data = getCase(doc, n);
...
function firstAdmTypeMissing(doc, admType) {
  for (let n = 1; n <= 5; n++) {
...
function firstFieldFilledTarget(doc, fieldIds) {
  for (let n = 1; n <= 5; n++) {
    const data = getCase(doc, n);
...
function firstUnfilledInCase(doc, caseNum, fieldIds) {
  const data = getCase(doc, caseNum);
  return fieldIds.find((f) => !isFilled(data[f])) || fieldIds[0];
}

function resolveNavRule(rule, doc) {
...
export function resolveCheckNavigation(ruleId, doc) {
