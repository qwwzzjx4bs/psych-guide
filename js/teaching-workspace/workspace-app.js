...
import {
  loadAutosave, saveAutosave, exportDocumentFile, pickAndImportFile,
} from './document-store.js';
import { loadChecklistRules } from './checklist-engine.js';
import { didUseEmbedFallback } from '../shitei-workspace/json-loader.js';
import {
...
import { PortalPalette } from '../shitei-workspace/portal-palette.js';
import { ReferencePane } from '../shitei-workspace/reference-pane.js';
import { VersionDrawer } from '../shitei-workspace/version-drawer.js';
import { PreviewDrawer } from './preview-drawer.js';
import { STEP_TOOL_HINTS } from './step-hints.js';

const UNDO_DEBOUNCE_MS = 800;

export class TeachingWorkspaceApp {
  constructor() {
    this.doc = null;
    this.saveTimer = null;
    this.dirty = false;
    ...
    this.wizard = new WizardPane(document.getElementById('wizardPane'), {
      ...
      onPreviewText: () => this.showPreview('text'),
      onPreviewMd: () => this.showPreview('md'),
    });

    this.checklist = new ChecklistPane(document.getElementById('checkPane'), {
      getDoc: () => this.getDoc(),
      getCurrentCaseId: () => this.wizard.getCurrentCaseId(),
    ...
    this.palette = new PortalPalette(document.getElementById('palettePane'), {
      ...
      favKey: 'teachingWorkspaceFavorites',
      relevantFlag: 'teachingCaseRelevant',
  ...
  applyDocument(doc, opts = {}) {
    ...
    if (titleInput) titleInput.value = doc.meta.title || '無題の症例集';
  ...
  updateStepCopyButton(step) {
    ...
    if (btn) btn.classList.toggle('hidden', step < 2);
  }

  openStepCopyModal() {
    const step = this.wizard.getCurrentStep();
    const caseId = this.wizard.getCurrentCaseId();
    if (step < 2) return;
    ...
    const doc = this.getDoc();
    list.innerHTML = doc.cases
      .filter((c) => c.id !== caseId)
      .map((c) => `<label class="step-copy-label"><input type="checkbox" value="${c.id}" checked> ${escapeHtml(c.label)}</label>`)
      .join('');
  ...
  bindStepCopyModal() {
    ...
    document.getElementById('stepCopyConfirm')?.addEventListener('click', () => {
      ...
      const result = copyStepToCases(doc, caseId, step, targets);
      if (result.applied > 0) {
  ...
  showPreview(mode) {
    const doc = this.getDoc();
    const c = this.wizard.getCurrentCase();
    if (!c) return;
    if (mode === 'md') this.previewDrawer.showMarkdown(c, doc.meta);
    else this.previewDrawer.showText(c, doc.meta);
  }
  ...
  bindHeader() {
    ...
    document.getElementById('titleInput')?.addEventListener('change', (e) => {
      const doc = this.getDoc();
      doc.meta.title = e.target.value || '無題の症例集';
    ...
    document.getElementById('narrowDismiss')?.addEventListener('click', () => {
      sessionStorage.setItem('teachingNarrowDismissed', '1');
  ...
  exportFile() {
    this.flushUndoCapture();
    const doc = this.getDoc();
    exportDocumentFile(doc, `${doc.meta.title || 'cases'}.tcases`);
  ...
  checkViewport() {
    ...
    if (!banner) return;
    if (window.innerWidth < 1280 && !sessionStorage.getItem('teachingNarrowDismissed')) {
...
document.addEventListener('DOMContentLoaded', () => {
  const app = new TeachingWorkspaceApp();
  app.init().catch((err) => {
    console.error(err);
